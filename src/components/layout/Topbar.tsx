    ...
    <header className="sticky top-0 z-10 bg-[#080c12]/96 backdrop-blur-sm border-b border-slate-800/60 px-5 md:px-6 py-0">
      <div className="flex items-center gap-3 h-[52px]">
        ...
        <div className="min-w-0 shrink-0">
          {breadcrumb && (
            <p className="text-[10px] text-slate-600 mb-0.5 leading-none">{breadcrumb}</p>
          )}
          <h1 className="text-white font-bold text-[16px] tracking-tight leading-none">{title}</h1>
          {subtitle && (
            <p className="text-slate-500 text-[11px] mt-0.5 leading-none truncate max-w-xs">{subtitle}</p>
          )}
        </div>
        ...
        <div className="flex-1 max-w-[240px] hidden lg:block ml-3">
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Buscar..."
              readOnly
              className="w-full pl-8 pr-14 py-1.5 bg-slate-800/40 border border-slate-700/40 text-slate-500 placeholder-slate-700 text-[12px] rounded-lg cursor-default focus:outline-none select-none"
            />
            <div className="absolute right-2 flex items-center gap-0.5 pointer-events-none">
              <kbd className="text-[9px] text-slate-700 bg-slate-800 px-1 py-0.5 rounded font-mono leading-none border border-slate-700/60">⌘</kbd>
              <kbd className="text-[9px] text-slate-700 bg-slate-800 px-1 py-0.5 rounded font-mono leading-none border border-slate-700/60">K</kbd>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 ml-auto shrink-0">
          ...
          <button className="relative w-8 h-8 flex items-center justify-center text-slate-500 hover:text-slate-200 hover:bg-slate-800/60 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500/40">
            ...
            <span className="absolute -top-0.5 -right-0.5 min-w-[14px] h-[14px] bg-blue-500 text-[8px] font-bold text-white rounded-full flex items-center justify-center px-0.5 ring-1 ring-[#080c12]">
              4
            </span>
          </button>
          ...
          {lucroHoje !== undefined && lucroHoje !== null && (
            <div className="hidden md:flex items-center gap-2.5 ml-1 pl-3 border-l border-slate-800/60">
              <div className="text-right leading-none">
                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider">Lucro Hoje</p>
                <p className={`text-[13px] font-bold tabular-nums mt-0.5 ${lucroHoje >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
