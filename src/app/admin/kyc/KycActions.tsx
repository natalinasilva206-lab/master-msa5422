    ...
    <>
      <div className="relative" ref={menuRef}>
        <button
          ...
          disabled={pending}
          className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-300 hover:bg-slate-800/60 border border-slate-700/40 transition-colors disabled:opacity-40"
        >
        ...
        {showMenu && (
          <div className="absolute right-0 top-9 z-30 w-48 bg-[#0d1117] border border-slate-800/80 rounded-xl shadow-2xl py-1 overflow-hidden">
            <button onClick={() => openModal('pessoal')} className="w-full text-left px-3 py-2.5 text-[12px] text-slate-300 hover:bg-slate-800/60 flex items-center gap-2.5 transition-colors">
              <svg className="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
              Ver Detalhes
            </button>
            {canAction && (
              <>
                <button onClick={() => { startApprove(() => approveMerchant(merchant.id)); setShowMenu(false) }} className="w-full text-left px-3 py-2.5 text-[12px] text-emerald-400 hover:bg-slate-800/60 flex items-center gap-2.5 transition-colors">
                ...
                <button onClick={() => openModal('pessoal', true)} className="w-full text-left px-3 py-2.5 text-[12px] text-amber-400 hover:bg-slate-800/60 flex items-center gap-2.5 transition-colors">
                ...
                <button onClick={() => { startCall(() => requestCall(merchant.id)); setShowMenu(false) }} className="w-full text-left px-3 py-2.5 text-[12px] text-blue-400 hover:bg-slate-800/60 flex items-center gap-2.5 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                  Solicitar Call
                </button>
                <div className="my-1 border-t border-slate-800/60" />
                <button onClick={() => { startReject(() => rejectMerchant(merchant.id)); setShowMenu(false) }} className="w-full text-left px-3 py-2.5 text-[12px] text-red-400 hover:bg-slate-800/60 flex items-center gap-2.5 transition-colors">
