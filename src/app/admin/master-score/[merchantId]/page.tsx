...
interface Props { params: { merchantId: string }; searchParams: { auditPage?: string } }

export default async function MasterScoreDetalhe({ params, searchParams }: Props) {
  const session = await getServerSession(authOptions)
  if ((session?.user as any)?.role !== 'ADMIN') redirect('/cliente/dashboard')

  const auditPage = Math.max(1, parseInt(searchParams?.auditPage ?? '1'))

  const [merchant, auditCount] = await Promise.all([
    ...
    prisma.masterScoreAudit.count({ where: { merchantId: params.merchantId } }),
  ])
          ...
          <div className="bg-slate-900/60 border border-slate-800/70 rounded-xl overflow-hidden">
            <div className="px-5 py-3.5 border-b border-slate-800/60 flex items-center justify-between">
              ...
              <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full border border-slate-700/40 bg-slate-800/60 text-slate-400">
                {auditCount} registro{auditCount !== 1 ? 's' : ''}
              </span>
            </div>
            <div className="divide-y divide-slate-800/40">
            ...
            {auditTotalPages > 1 && (
              <div className="px-5 py-3 border-t border-slate-800/40 flex items-center justify-between text-[12px] text-slate-500">
                ...
                <div className="flex gap-2">
                  {auditPage > 1 && (
                    <a href={`?auditPage=${auditPage - 1}`} className="px-3 py-1.5 bg-slate-800 border border-slate-700/50 rounded-lg hover:bg-slate-700 transition-colors text-slate-300">← Anterior</a>
                  )}
                  {auditPage < auditTotalPages && (
                    <a href={`?auditPage=${auditPage + 1}`} className="px-3 py-1.5 bg-slate-800 border border-slate-700/50 rounded-lg hover:bg-slate-700 transition-colors text-slate-300">Próxima →</a>
                  )}
                </div>
              </div>
            )}
