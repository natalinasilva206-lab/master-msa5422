    ...
    <div>
      ...
      <div className="p-4 xl:p-6 space-y-4">
        ...
        <section className="bg-slate-900/60 border border-slate-800/70 rounded-xl overflow-hidden">
          ...
          {vendas.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-slate-700 px-6 text-center">
              ...
              <p className="text-[11.5px] text-slate-700 mt-1.5 max-w-xs">
                As transações dos seus clientes aparecerão aqui automaticamente após a integração com a API.
              </p>
              {/* Next steps */}
              <div className="mt-6 w-full max-w-sm space-y-2 text-left">
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-3 text-center">Próximos passos</p>
                {[
                  { step: '1', title: 'Obtenha sua API Key', desc: 'Acesse Integrações / API e copie sua chave de produção.', href: '/cliente/integracoes' },
                  { step: '2', title: 'Envie um POST /api/v1/sales', desc: 'Registre cada venda aprovada pelo seu gateway com o payload JSON documentado.', href: '/cliente/integracoes' },
                  { step: '3', title: 'Configure um Webhook', desc: 'Receba notificações em tempo real de pagamentos, saques e disputas.', href: '/cliente/minha-conta' },
                ].map((item) => (
                  <Link key={item.step} href={item.href} className="flex items-start gap-3 bg-slate-800/40 hover:bg-slate-800/60 border border-slate-700/30 rounded-xl px-4 py-3 transition-colors group">
                    <div className="w-6 h-6 rounded-full bg-blue-600/20 text-blue-400 text-[11px] font-bold flex items-center justify-center shrink-0 mt-0.5 group-hover:bg-blue-600/30 transition-colors">
                      {item.step}
                    </div>
                    <div>
                      <p className="text-[12px] font-semibold text-slate-300 group-hover:text-white transition-colors">{item.title}</p>
                      <p className="text-[10.5px] text-slate-600 mt-0.5">{item.desc}</p>
                    </div>
                    <svg className="w-3.5 h-3.5 text-slate-700 group-hover:text-slate-400 transition-colors ml-auto shrink-0 mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
