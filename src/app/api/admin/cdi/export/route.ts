...
function buildExtrato(logs: LogRow[], _currentBalance: number): ExtratoRow[] {
  ...
  for (const log of ordered) {
    ...
    switch (log.action) {
      case 'ADD_TO_CDI':    running += amount; break
      case 'CDI_WITHDRAW':
      case 'CDI_EARLY_APPROVED': running -= amount; break
      case 'CDI_CREDIT':   running += amount; break
    }
...
function buildCsv(rows: ExtratoRow[], merchantName: string): string {
  ...
  const dataRows = rows.map((r) => [
    fmtDate(r.createdAt), r.tipo,
    r.valor > 0 ? r.valor.toFixed(2) : '—',
    r.saldoAntes.toFixed(2), r.saldoDepois.toFixed(2),
...
<div class="header">
  <h1>Extrato CDI</h1>
  <div class="sub">${merchantName} · ${periodLabel} · Exportado pelo Administrador</div>
</div>
<div class="meta">
...
<table>
  <thead>
    <tr>
      <th>Data/Hora</th><th>Tipo</th><th>Valor</th><th>Saldo Antes</th>
      <th>Saldo Depois</th><th>Taxa CDI</th><th>Base de Cálculo</th><th>Status</th>
    </tr>
  </thead>
  <tbody>${rowsHtml}</tbody>
</table>
<div class="footer">
  Documento interno — Master Pagamentos. Gerado via painel administrativo.
</div>
...
export async function GET(req: NextRequest) {
  ...
  if (!merchant) return NextResponse.json({ error: 'Merchant não encontrado' }, { status: 404 })
  ...
  const fileName = `extrato-cdi-${merchant.name.replace(/\s+/g, '-').toLowerCase()}-${now.toISOString().slice(0, 10)}`
